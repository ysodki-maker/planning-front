...
function validate(form) {
  ...
  // dates optionnelles — on valide seulement la cohérence si les deux sont renseignées
...
export default function ProjectForm({ initial = {}, users = [], onSave, onCancel, loading }) {
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.form}>
      ...
      <div className={styles.row2}>
        <Field label="Nom du projet" required error={errors.name}>
          <Input
            ...
            placeholder="Ex : Relevé Villa Californie" error={errors.name}
          />
        </Field>
      ...
      {/* Dates (optionnelles) */}
      <div className={styles.row2}>
        <Field label="Date de début" hint="Optionnel" error={errors.start_date}>
          <Input
            type="date" value={form.start_date}
            onChange={set('start_date')} error={errors.start_date}
          />
        </Field>
        <Field label="Date de fin" hint="Optionnel" error={errors.end_date}>
          <Input
            type="date" value={form.end_date}
            onChange={set('end_date')} min={form.start_date || undefined}
      ...
      <div className={styles.row2}>
        <Field label="Heure de début" hint="Optionnel — ex : 08:30" error={errors.heure_debut}>
          <Input
            ...
            onChange={set('heure_debut')} error={errors.heure_debut}
          />
        </Field>
        <Field label="Heure de fin" hint="Optionnel — ex : 17:00" error={errors.heure_fin}>
          <Input
            type="time" value={form.heure_fin}
            onChange={set('heure_fin')} min={form.heure_debut || undefined}
      ...
      {/* Localisation */}
      <Field label="Localisation" hint="Adresse précise, coordonnées ou lien Maps — optionnel">
        <Textarea
          value={form.localisation} onChange={set('localisation')}
          placeholder="Ex : Villa Californie, Bd de la Corniche, Casablanca&#10;ou https://maps.google.com/..."
          rows={2}
        />
      </Field>

      {/* Assignation (optionnelle) */}
      <Field label="Assigner à" hint="Optionnel">
      ...
      <Field label="Description" hint="Optionnel">
        <Textarea
          ...
          placeholder="Détails du projet…" rows={2}
        />
      </Field>
